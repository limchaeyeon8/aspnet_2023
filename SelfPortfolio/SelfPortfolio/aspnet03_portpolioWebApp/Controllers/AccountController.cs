...
namespace aspnet02_boardapp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Profile(string userName)
        {
            ...
            if (curUser == null)    // null이면(사용자가 없으면)
            {
        ...
        public async Task<IActionResult> Profile(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                user.PasswordHash = _userManager.PasswordHasher.HashPassword(user, model.Password);   // 비밀번호 변경은 따로~~
            ...
            return View(model);     // 프로필변경 실패하면 그 화면 그대로 유지
        }
    }
}
